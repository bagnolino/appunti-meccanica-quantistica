...
Per essere equivalenti le due visioni, deve valere il teorema di Wigner e quindi abbiamo:
\[
...
Occupiamoci ora di stati, di cui non disponiamo tutta l'informazione, ma solo la probabilità che sia in certo stato.
\begin{definition}
...
Per trattare gli stati misti è comodo associare a un certo oggeto matematico chiamato \textit{operatore densità} o \textit{matrice densità} \(\rho\).
...
\begin{definition}
    Un operatore \(\rho : \mathcal{H}\to \mathcal{H}\) è \textit{operatore densità} sse:
    \begin{equation}
...
\begin{remark}
    ...
    I due diversi mix sono indistinguibili perché restiruiscono gli stessi valori medi.
\end{remark}
