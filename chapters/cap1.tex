...
\end{remark}
\begin{definition}
    Per \(\forall a \in \sigma_d(A) , \; \exists\) autospazio \(\mathcal{H}\mathcal{P}_a\subseteq\mathcal{H}\) di autovettori, 
    la cui dimensione \(d(a):= \operatorname{dim}\mathcal{A}\mathcal{P}_a\) è detta \textit{degenerazione} di \(a in \sigma_d(A)\) .\\
    Se \(d(a)=1,\; a \) è detto \textit{non-degenere} e \(\Psi\in \mathcal{H}\mathcal{P}_a\) è unico a meno riscalamenti. Se \(d(a)>1, \; a\) è detto \textit{degenere}.
\end{definition}
...
\begin{theorem}
    ...
    \begin{equation}
        ...
        \qquad
        \mathcal{H} = \bigoplus_{a \in \sigma_d(A)} \mathcal{H}\mathcal{P}_a.
    \end{equation}
\end{theorem}
...
Sia \(A\) autoaggiunto e \(a\in \sigma_d(A)\). Indichiamo con \(\mathcal{H}\mathcal{P}_a\) l'autospazio relativo ad \(a\) e con \(d(a)= \dim \mathcal{H}\mathcal{P}_a\) la degenerazione. 
Fissata una base ON \(\{\ket{a,r}\}_{r=1}^{d(a)}\) di \(\mathcal{H}\mathcal{P}_a\), il proiettore su \(\mathcal{H}\mathcal{P}_a\) è
\begin{equation}
    \mathcal{P}\mathcal{P}_a \equiv \mathcal{P}_{\mathcal{H}\mathcal{P}_a}= \sum_{r=1}^{d(a)} \dyad{a,r}{a,r}.
\end{equation}
    

Se lo spettro è puramente discreto, \(\sigma(A)= \sigma_d(A)\), ed esiste una base ON di autovettori di \(A\), allora valgono:
\[
    \mathbb{I}= \sum_{a\in \sigma(A)} \mathcal{P}\mathcal{P}_a\,,\qquad
    A= \sum_{a\in \sigma(A)} a\,\mathcal{P}\mathcal{P}_a.
\]

Se \(\dim \mathcal{H} < \infty\), per ogni \(a\in \sigma(A)\) il proiettore si può esprimere come:
\[
    \mathcal{P}\mathcal{P}_a= \prod_{\substack{a'\in \sigma(A)\\ a'\neq a}} \frac{a'\,\mathbb{I}- A}{a'-a}.
\]
...
\begin{postulato5}
    ...
    allora lo stato al tempo \(t+\epsilon\) per \(\epsilon\) abbastanza piccolo tende a \(\mathcal{P}\mathcal{P}_a\ket{\Psi}\).
\end{postulato5}
...
Al tempo \(t\) misuro \(\mathcal{A}\) e ottengo \(a \in \sigma_d(\mathcal{A})\), al tempo \(t+\epsilon\) avrò lo stato \(\mathcal{P}\mathcal{P}_a\ket{\Psi}\) e misuro \(\mathcal{B}\) ottenendo \(b \in \sigma_d(\mathcal{B})\),
avrò quindi lo stato \(\mathcal{P}_b\mathcal{P}\mathcal{P}_a\ket{\Psi}\) al tempo \(t+2\epsilon\) e infine misuro nuovamente \(\mathcal{A}\) .
...
\begin{definition}
    \(\vec{A} = (A_1, \dots, A_n)\) è \textit{insieme completo di osservabili compatibili (I.C.O.C.)} sse:
    \[
...
Consideriamo ora l'evoluzioni causale, non consideriamo i processi di misura.
...
\begin{definition}
    ...
    Equivalentemente, questo vale sse:
    \begin{equation}
