...
\begin{align}
    \tilde{A}_1 &= A_1 \otimes \id_{\mathcal{H}_2} \\
    \tilde{B}_2 &= \id_{\mathcal{H}_1} \otimes B_2
\end{align}
...
\[
R(\vec{\omega})^{-1}\vec{X} \approx \left( \id_3 - \sum_j \omega_j T_j \right)\vec{X}
\]
Dalla relazione tra trasformazioni infinitesime e commutatori (vedi equazione \ref{eq: simmetria}), otteniamo:
\[
...
\[
    \vec{X} = \vec{X}_{\vec{x}} \otimes \id_{\mathcal{H}_s}
\]
\[
    \vec{P} = \vec{P}_{\vec{x}} \otimes \id_{\mathcal{H}_s}
\]
dove \(\id_{\mathcal{H}_s}\) è l'identità nello spazio dello spin (questi operatori "non fanno nulla" nello spazio \(\mathcal{H}_s\)).

Analogamente, i 3 operatori di spin agiscono solo su \(\mathcal{H}_s\):
\[
    \vec{S} = \id_{\mathcal{H}_{\vec{x}}} \otimes \vec{S}_s
\]

Poiché \(\vec{S}^2 \propto \id\), è necessario utilizzare \(S_z\) come osservabile per completare l'I.C.O.C. nel sottospazio dello spin. Lo spettro di \(S_z\) è dato da:
\begin{equation}
...
Concentriamoci ora su particelle da spin \(s = \frac{1}{2}\), avremo dunque \(\vec{S}^2= \frac{3}{4}\hbar^2 \id\) , \(m \in \left\{ -\frac{1}{2},\frac{1}{2} \right\}\) e lo spazio di Hilbert dello spin \(\mathcal{H}_s \cong\mathbb{C}^2\).\\
...
\begin{itemize}
    ...
    \item \(\left\{ \sigma_i,\sigma_j \right\}= 2\delta_{ij}\id\)
    \item \(\Tr(\sigma_k)=0\)
    \item \(\det\sigma_k= -1\)
\end{itemize}    
\end{multicols}
...
\[
    \begin{pmatrix}
        ...
        b+ic & d
    \end{pmatrix}
    = \frac{a+d}{2} \id + b \sigma_1 + c \sigma_2 + \frac{a-d}{2} \sigma_3, \quad a,b,c,d \in \mathbb{R}
\]
...
\[
(\hat{n} \cdot \vec{\sigma})^2 = \sum_{k} n_k^2 \sigma_k^2+ \sum_{i < j} n_i n_j \{\sigma_i, \sigma_j\}=\id
\]
...
\[
(\vec{v} \cdot \vec{\sigma})(\vec{u} \cdot \vec{\sigma}) = \sum_{i,j} v_i u_j \sigma_i \sigma_j = \sum_{i,j} v_i u_j \left(\delta_{ij}\id + i \sum_k \epsilon_{ijk}\sigma_k\right)
\]
Il primo termine restituisce il prodotto scalare, il secondo (antisimmetrico) restituisce il prodotto vettoriale:
\begin{equation}
(\vec{v} \cdot \vec{\sigma})(\vec{u} \cdot \vec{\sigma}) = (\vec{v} \cdot \vec{u}) \id + i (\vec{v} \times \vec{u}) \cdot \vec{\sigma}
\end{equation}
...
\]
Per calcolare l'esponenziale, sfruttiamo la proprietà \((\hat{n}\cdot\vec{\sigma})^2 = \id\),
...
\begin{align*}
    ...
    &= \sum_{k=0}^{\infty} \frac{(-1)^k}{(2k)!} \left(\frac{\theta}{2}\right)^{2k} \id - i \sum_{k=0}^{\infty} \frac{(-1)^k}{(2k+1)!} \left(\frac{\theta}{2}\right)^{2k+1} (\hat{n}\cdot\vec{\sigma}).
\end{align*}
Riconoscendo le serie di Taylor per il coseno e il seno, otteniamo la formula fondamentale per la rotazione dello spin:
\begin{equation}
    U(\theta \hat{n}) = \cos(\frac{\theta}{2})\id - i \sin(\frac{\theta}{2}) (\hat{n}\cdot\vec{\sigma}).
\end{equation}
...
\[
    U(2\pi \hat{n}) = \cos(\pi)\id - i \sin(\pi)(\hat{n}\cdot\vec{\sigma}) = -\id.
\]
Non otteniamo l'identità \(\id\), ma \(-\id\). Lo stato ruotato differisce da quello iniziale per una fase globale:
\[
...
I primi agiscono sulle componenti della funzione d'onda ma "banalmente" sullo spazio degli spin (sono proporzionali all'identità \( \id_{2\times 2} \) nello spazio dello spin).
...
\[
    \vec{J} = \vec{L} \otimes \id_s + \id_{\vec{x}} \otimes \vec{S}
\]
...
\[
    U(\vec{\omega}) = e^{-\frac{i}{\hbar} \vec{J} \cdot \vec{\omega}} = e^{-\frac{i}{\hbar} (\vec{L} \cdot \vec{\omega} \otimes \id_s + \id_{\vec{x}} \otimes \vec{S} \cdot \vec{\omega})}
\]

Poiché gli operatori agiscono su spazi disgiunti, essi commutano: \( [\vec{L} \otimes \id_s, \id_{\vec{x}} \otimes \vec{S}] = 0 \). Possiamo quindi utilizzare la proprietà \( e^{A+B} = e^A e^B \) per fattorizzare l'operatore:
\[
    U(\vec{\omega}) = \left( e^{-\frac{i}{\hbar} \vec{L} \cdot \vec{\omega}} \otimes \id_s \right) \left( \id_{\vec{x}} \otimes e^{-\frac{i}{\hbar} \vec{S} \cdot \vec{\omega}} \right)
...
\end{equation}
dove la somma è intesa come somma diretta di operatori estesi ai prodotti tensoriali (\( \vec{J}_1 \otimes \id + \id \otimes \vec{J}_2 \)).
...
Consideriamo, ad esempio, l'addizione di due momenti angolari con numeri quantici $j_1 = \frac{3}{2}$ e $j_2 = 1$.
...
Ricordiamo che gli operatori di innalzamento e abbassamento totali sono definiti come la somma degli operatori sui singoli spazi:
\begin{equation}
    J_{\pm} = J_{1\pm} + J_{2\pm}
\end{equation}
L'azione di questi operatori sugli autostati è governata dai coefficienti \( \mathcal{N}_{\pm}(j,m) \):
\begin{equation}
    J_{\pm}\ket{j,m} = \mathcal{N}_{\pm}(j,m)\ket{j, m\pm 1} = \hbar\sqrt{j(j+1) - m(m\pm 1)}\ket{j, m\pm 1}
\end{equation}

Partiamo dallo stato con la somma dei numeri quantici magnetici massima possibile. In questo caso \( m_{max} = m_{1,max} + m_{2,max} = \frac{3}{2} + 1 = \frac{5}{2} \).
Osserviamo l'azione di \( J_+ \) su tale stato:
\[
    J_+ \ket{\frac{3}{2}; 1} = (J_{1+} + J_{2+}) \ket{\frac{3}{2}; 1} = 0
\]
È nullo perché le proiezioni \( m_1 \) e \( m_2 \) sono già ai loro valori massimi e non possono essere alzate ulteriormente.
Sappiamo che \( J_+ \) annichilisce un autostato di \( J_z \) se e solo se esso è un autostato di \( \vec{J}^2 \) con \( m=j \).
\begin{equation}
    J_+ \ket{j,m} = 0 \implies m=j
\end{equation}
Quindi identifichiamo questo stato come il primo vettore della nuova base accoppiata, quello con spin totale massimo:
\begin{equation}
    \ket{m_1=\frac{3}{2}; m_2=1} \equiv \ket{J=\frac{5}{2}, m=\frac{5}{2}}
\end{equation}

Dato che abbiamo fissato \( J=\frac{5}{2} \), il multipletto deve contenere tutti gli stati con \( m \in \{-\frac{5}{2}, \dots, \frac{5}{2}\} \).
Per trovare gli altri stati del multipletto, applichiamo l'operatore di abbassamento \( J_- = J_{1-} + J_{2-} \).

Calcoliamo il primo stato discendente \( \ket{J=\frac{5}{2}, m=\frac{3}{2}} \):
\[
    \ket{\frac{5}{2}, \frac{3}{2}} = \frac{J_-}{\mathcal{N}_-(\frac{5}{2}, \frac{5}{2})} \ket{\frac{5}{2}, \frac{5}{2}}
\]
Sostituendo l'espressione in termini delle singole particelle:
\[
    = \frac{1}{\mathcal{N}_-(\frac{5}{2}, \frac{5}{2})} \left( J_{1-}\ket{\frac{3}{2}; 1} + J_{2-}\ket{\frac{3}{2}; 1} \right)
\]
Calcoliamo separatamente i coefficienti di normalizzazione:
\begin{itemize}
    \item Per lo spin totale: \( \mathcal{N}_-(\frac{5}{2}, \frac{5}{2}) = \hbar\sqrt{\frac{5}{2}(\frac{7}{2}) - \frac{5}{2}(\frac{3}{2})} = \hbar\sqrt{\frac{35-15}{4}} = \hbar\sqrt{5} \)
    \item Per la particella 1: \( \mathcal{N}_-(\frac{3}{2}, \frac{3}{2}) = \hbar\sqrt{\frac{3}{2}(\frac{5}{2}) - \frac{3}{2}(\frac{1}{2})} = \hbar\sqrt{3} \)
    \item Per la particella 2: \( \mathcal{N}_-(1, 1) = \hbar\sqrt{1(2) - 1(0)} = \hbar\sqrt{2} \)
\end{itemize}

Sostituendo i valori ottenuti:
\[
    \ket{\frac{5}{2}, \frac{3}{2}} = \frac{1}{\hbar\sqrt{5}} \left( \hbar\sqrt{3}\ket{\frac{1}{2}; 1} + \hbar\sqrt{2}\ket{\frac{3}{2}; 0} \right)
\]
Semplificando \( \hbar \), otteniamo l'espressione dello stato normalizzato nella base disaccoppiata:
\begin{equation}
    \ket{J=\frac{5}{2}, m=\frac{3}{2}} = \sqrt{\frac{3}{5}} \ket{\frac{1}{2}; 1} + \sqrt{\frac{2}{5}} \ket{\frac{3}{2}; 0}
\end{equation}

\noindent
L'operatore $J_+$ mappa un sottospazio di dimensione 2 (quello con $m_{tot} = 3/2$) in uno di dimensione 1 (quello con $m_{tot} = 5/2$). Di conseguenza, $J_+$ deve avere un \textit{kernel} non banale in questo sottospazio: deve esistere cioè una combinazione lineare di vettori che viene annichilita da $J_+$.

Cerchiamo i coefficienti $\alpha, \beta$ tali per cui la combinazione:
\[
    \ket{\psi} = \alpha \ket{\frac{1}{2}; 1} + \beta \ket{\frac{3}{2}; 0}
\]
soddisfi la condizione $J_+ \ket{\psi} = 0$ (condizione di \textit{highest weight} per il multipletto con $j=3/2$) e la normalizzazione $|\alpha|^2 + |\beta|^2 = 1$.
Scrivendo $J_+ = J_{1+} + J_{2+}$, l'azione sullo stato è:
\[
    0 = \alpha J_{1+} \ket{\frac{1}{2}; 1} + \underbrace{\alpha J_{2+} \ket{\frac{1}{2}; 1}}_{=0 \, (m_2=1 \text{ max})} + \underbrace{\beta J_{1+} \ket{\frac{3}{2}; 0}}_{=0 \, (m_1=3/2 \text{ max})} + \beta J_{2+} \ket{\frac{3}{2}; 0}
\]
Utilizzando l'azione degli operatori a scala $J_{\pm}\ket{j,m} = \hbar\sqrt{j(j+1) - m(m\pm 1)}\ket{j, m\pm 1}$:
\begin{align*}
    J_{1+} \ket{\frac{1}{2}} &= \hbar\sqrt{3} \ket{\frac{3}{2}} \\
    J_{2+} \ket{0} &= \hbar\sqrt{2} \ket{1}
\end{align*}
L'equazione diventa:
\[
    \alpha \sqrt{3} \ket{\frac{3}{2}; 1} + \beta \sqrt{2} \ket{\frac{3}{2}; 1} = 0 \implies \alpha\sqrt{3} + \beta\sqrt{2} = 0
\]
Da cui ricaviamo la relazione $\beta = -\alpha\sqrt{\frac{3}{2}}$. Imponendo la normalizzazione:
\[
    |\alpha|^2 + \frac{3}{2}|\alpha|^2 = 1 \implies \alpha^2 \frac{5}{2} = 1 \implies \alpha = \sqrt{\frac{2}{5}}
\]
Scegliendo la fase in modo coerente (segno reale positivo per il primo coefficiente), otteniamo i coefficienti di Clebsch-Gordan per questo stato:
\[
    \alpha = \sqrt{\frac{2}{5}}, \qquad \beta = - \sqrt{\frac{3}{5}}
\]
Quindi, questa particolare combinazione lineare viene annichilita da $J_+$, identificando lo stato con spin totale $j=3/2$ e proiezione massima:
\begin{equation}
    \ket{j=\frac{3}{2}, m=\frac{3}{2}} = \sqrt{\frac{2}{5}} \ket{\frac{1}{2}; 1} - \sqrt{\frac{3}{5}} \ket{\frac{3}{2}; 0}
\end{equation}

\begin{remark}
    Si noti che questo stato è ortogonale allo stato del multipletto $j=5/2$ con la stessa proiezione magnetica $m=3/2$, che (tramite applicazione di $J_-$ allo stato $j=5/2, m=5/2$) risulta essere:
    \[
        \ket{j=\frac{5}{2}, m=\frac{3}{2}} = \sqrt{\frac{3}{5}} \ket{\frac{1}{2}; 1} + \sqrt{\frac{2}{5}} \ket{\frac{3}{2}; 0}
    \]
    Verifichiamo l'ortogonalità:
    \[
        \braket{\frac{5}{2}, \frac{3}{2}}{\frac{3}{2}, \frac{3}{2}} = \left(\sqrt{\frac{3}{5}}\right)\left(\sqrt{\frac{2}{5}}\right) + \left(\sqrt{\frac{2}{5}}\right)\left(-\sqrt{\frac{3}{5}}\right) = 0
    \]
    Se così non fosse stato, il vettore trovato non sarebbe stato annichilito da $J_+$.
\end{remark}
Proseguendo applicando gli operatori \(J_\pm \) è possibile trovare i coefficienti rimanenti.
\begin{remark}
    La fase globale non conta fisicamente ($\alpha$ e $\beta$ possono avere una fase comune), ma la fase relativa (il segno meno) è fondamentale per garantire l'ortogonalità. Per convenzione si sceglie un segno per i coefficienti e si rimane coerenti (convenzione di Condon-Shortley: coefficienti reali).
\end{remark}
...
\begin{itemize}
    ...
    \item 
    \(
    \hat{P}_{12}^2 = \id \implies \hat{P}_{12} = \hat{P}_{12}^{-1} = \hat{P}_{12}^\dagger \implies \hat{P}_{12} \text{ è a.a.}
    \)
...
\begin{equation}
    \frac{1}{\sqrt{2}}\left( \ket{a_n}\otimes\ket{a_m} + \ket{a_m}\otimes \ket{a_n} \right)  \in \mathcal{H}_S
\end{equation}
Mentre per i fermioni è quella antisimmetrica:
\begin{equation}
    \frac{1}{\sqrt{2}}\left( \ket{a_n}\otimes\ket{a_m} - \ket{a_m}\otimes \ket{a_n} \right)  \in \mathcal{H}_A
\end{equation}
...
\subsubsection{Effetto fisico di funzioni d'onda simmetriche e antisimmetriche}
