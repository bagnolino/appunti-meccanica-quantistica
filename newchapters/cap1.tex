...
\begin{example}
    ...
    \begin{equation}
        \chi\mapsto \phi_\psi(\chi):= \ip{\psi}{\chi}
    \end{equation}
...
Da cui notiamo \(\ket{\psi}= \sum_{n=1}^{\infty}\ket{n}\braket{n}{\psi} \implies \id_n= \sum_{n=1}^{\infty} \dyad{n}{n}\)
detta \textbf{relazione di completezza}.

\begin{example}
    \[
    \braket{\psi}{\chi}= \matrixel{\psi}{\id}{\chi}=
...
\begin{equation}
    \int \dd{x_0} \dyad{x_0}{x_0}= \id_\mathcal{H} = \int \dd{p} \dyad{p}{p}
\end{equation}
...
\begin{example}
    ...
    \begin{equation}
        \id= \sum_{n,m} \ket{n}\bra{n} \id \ket{m}\bra{m}= \sum_{n,m} \ket{n}\delta_{nm}\bra{m}= \sum_n \ket{n}\bra{n}
    \end{equation}
\end{example}
...
\begin{example}
    ...
    \end{equation}
    L'aggiunto di un operatore equivale a trasporre la matrice e cogniugare i suoi elementi.
\end{example}
...
\begin{remark}
   ...
   \begin{equation}
     (\Delta A )_{\hat{a}, \Sigma} = \sqrt{\expval{(\mathcal{A}-\hat{a})^2}_\Sigma}   = \sqrt{\expval{(A-\hat{a}\id)^2}_{\Psi}}
   \end{equation}
\end{remark}
...
\end{theorem}
\begin{proof}
    \((\Delta \mathcal{A})^2_{\hat{a}, \Sigma} = \sum_{a \in \sigma(\mathcal{A})}(a-\hat{a})^2 P_{\Sigma}(a)\) è una somma di numeri non negativi.
    \[
...
\begin{definition}
    \(\hat{a} \in \mathbb{R}\) è \textit{autovalore} di \(A\) autoaggiunto se e solo se \(\exists \,\Psi \neq 0 \in D_A\) tale che \((A-\hat{a})\Psi=0\). \\
...
\begin{proof}
    \[
        (\Delta \mathcal{A})^2_{\hat{a},\Sigma}= \frac{\expval{(A-\hat{a}\id)^2}{\Psi}}{\norm{\Psi}^2}= 
        \frac{\braket{(A-\hat{a}\id)\Psi}{(A-\hat{a}\id)\Psi}}{\norm{\Psi}^2} = \frac{\norm{(A-\hat{a}\id)\Psi}^2}{\norm{\Psi}^2}
    \]
    \[
        (\Delta\mathcal{A})_{\hat{a},\Sigma} = 0 \iff (A-\hat{a}\id)\Psi=0 
    \]    
\end{proof}
...
\begin{remark}
    ...
    \begin{equation}
        \int \dd{x} \ket{x}\bra{x} = \id = \int \dd{p} \ket{p}\bra{p}
    \end{equation}
...
Si dimostra che \(\sigma_c(A) = \left\{ \text{ autovalori generalizzati } \right\}\) .
\begin{definition}
...
\begin{proof}
    Definiamo per comodità : \(\hat{A} = A- a \id\) e \(\hat{B} = B - b\id\) autoaggiunti.
    \[
...
\begin{example}
    ...
    per cui
    \[
        [X,P]\,\Psi(x)= (XP-PX)\Psi(x)= i\hbar\,\Psi(x) \quad \Rightarrow \quad [X,P]= i\hbar\,\id
    \]
    Il commutatore non è hermitiano, mentre \(-i[X,P]= \hbar\,\id\) è autoaggiunto. Applicando il teorema otteniamo
    \[
        (\Delta X)_{x_0,\Psi}\,(\Delta P)_{p_0,\Psi} \geq \frac{1}{2}\abs{\expval{[X,P]}_\Psi}
        = \frac{\hbar}{2} \abs{\expval{\id}_\Psi}= \frac{\hbar}{2}
    \]
\end{example}
...
\begin{remark}
    Se \(\mathcal{P}_{\tilde{\mathcal{H}}} \) è un proiettore, lo è anche \(\id- \mathcal{P}_{\tilde{\mathcal{H}}}\).
\end{remark}
\begin{remark}
...
\subsubsection{Teoria della misura in M.Q.}
...
\{ q_j, p_k \} = \delta_{jk}
\;\;\longrightarrow\;\;
\frac{1}{i\hbar}[Q_j, P_k] = \delta_{jk}\,\id
\]
