...
\begin{proof}
    \((\impliedby) \) 
    \[
        \left[ \Psi \right]\to \left[ e^{i\theta}U \Psi\right]= \left[ U\Psi \right] \quad A \to e^{i\theta}UAU^\dagger e ^{-i\theta}= UAU^\dagger
    \]
    \((\implies) \) supposto che \(\tau_U = \tau_{U'}: \ \ UAU^\dagger = U'AU'^\dagger\) abbiamo che:
    \[
       U^\dagger U A U^\dagger U'= U^\dagger U'A U'^\dagger U'  \implies AU^\dagger U'= U^\dagger U ' A
       \implies \left[ U^\dagger U', A \right]=0
    \] 
    Sfruttiamo il lemma: \(B\) t.c. \(\left[ B,A \right]= 0 , \ A \) a.a. \(\implies B = \lambda \id\).
    \[
        \implies U^\dagger U' = \lambda \id , \ \abs{\lambda} = 1 \implies U ' = e^{i\theta}U
    \]
\end{proof}
...
\begin{remark}
    \(\id\) è simmetria, composizione di simmetrie \(\tau_1 \circ \tau_2\) è simmetria e \(\tau^{-1}\) è simmetria.
\end{remark}
...
\[
    \tau \leftrightarrow \left[ U_\tau \right]; \quad id \leftrightarrow \left[ \id \right]  ;\quad \tau_1\circ \tau_2 \leftrightarrow \left[ U_{\tau_1} U_{\tau_2} \right]; \quad
...
\begin{definition}
    ...
    \begin{equation}
        \rho: G \to GL(V) \quad g \mapsto \rho(g)
    \end{equation}
    tale che:
    \begin{itemize}
        \item \(\rho(1)= \id\)
...
Possiamo sempre scegliere per \(\tau = id\) tra i \(\left[ \id \right] \) l'operatore \(\id\). Scelti \(U_{\tau_1}\) e \(U_{\tau_2}\),
...
\begin{definition}
    ...
    \[
        \rho(1) = e^{i\alpha}\id \qquad \rho(gh)= e^{i\theta(g,h)} \rho(g)\rho(h)
    \]
\end{definition}
...
\begin{example}
    ...
    \[
        U_1U_2 = iU_3, \qquad U_2U_1 = -iU_3, \qquad U_k^2 = \id
    \]

    Nessuna scelta di \(U_1, U_2, U_3\) soddisfa \(U_1U_2=U_3\) e \(U_k^2=\id\).
\end{example}
...
\end{theorem}
E' sempre possibile, dunque, secgliere una rappresentazione tale che \(U(0)= \id, \  U(s)U(s')= U(s+s')\) .

Se considero \(s \) infinitesimo, posso sviluppare in Taylor:
\begin{equation}
    U(s )\ket{\Psi}= U(0)\ket{\Psi}+ s \dv{U}{s} \eval_{s = 0} + \dots = \id - \frac{i}{\hbar}s Q    + \dots
\end{equation}
...
\begin{theorem}\label{theo: Stone}
    {\normalfont\textbf{di Stone}} Dato un gruppo a un parametro \(\left\{ U(s) \text{ unitari } , \ s \in \mathbb{R}\right\}\) tale che \(U(0)= \id\), \(U(s )U (s') = U(s+s')\) e sia continuo in \(s \), allora:
    \[
...
\[
    A(s) = 
    \left( \id - \frac{i}{\hbar}sQ + \dots \right)
    A
    \left( \id + \frac{i}{\hbar}sQ + \dots \right)
    = A - \frac{i}{\hbar}\left[ Q, A \right]s + \dots
\]

\begin{equation}\label{eq: simmetria}
...
\[
    U(a)\ket{\Psi}= \ket{\Psi'} \quad \Psi'(x)= \Psi(x-a) \quad X' = U(a)XU(a)^\dagger = X- a\id \quad U(a)P U(a)^\dagger = P
\]
...
Consideriamo la trasformazione di un vettore \(\vec{x} \in \mathbb{R}^3 \to \vec{x'}= R\vec{x}\), in cui consideriamo le matrici \(3\times3\) di rotazione \(R \) tali che \(R^\intercal R = \id_3\) detto \textit{gruppo ortogonale} \(O(3)\).\\
...
\begin{example}
    \(U(n)=\left\{ U \in \text{Mat}_{n\times n } : U^\dagger U = \id \right\}\) detto \textit{gruppo unitario}.\\
    \(SU(n) = \left\{ U \in U(n ) : \det U = +1 \right\}\) detto \textit{gruppo unitario speciale}.
\end{example}


Consideriamo ora rotazioni infinitesime della forma \(R = \id + s \delta R + O(s^2)\). Per la condizione \(R^\intercal R =\id\) otteniamo:
\[
    \id= (\id+s \delta R^\intercal + \dots )(\id+s \delta R+ \dots )= \id + s(\delta R^\intercal+\delta R )
...
\[
    \left[ J_i , J_j\right] = i\hbar\sum_k \epsilon_{ijk} J_k + c_{ij} \id 
\]
ma nel caso di \(so(3)\) si può sempre porre \(c_{ij}= 0 \ \forall\,i,j\) .\\
Inoltre, si ha il problema che per \(\vec{\omega}= (0 \ 0 \ \pi)\) avrò:
\[
    R(\vec{\omega })R(\vec{\omega}) = \id  \quad  \text{ ma }\quad  U_{R(\vec{\omega}) }U_{R(\vec{\omega}) }= \pm \id 
\]
...
\begin{example}
    ...
    \[
        \mathcal{H} = L_2(\mathbb{R}^3) \quad R \in SO(3)\qquad
    ...
    Sviluppando \(R(-\vec{\omega})=\id-\sum_k\omega_k T_k+\dots\) con 
    \((T_k)_{j\ell}=-\epsilon_{k j \ell}\), si ha:
    \[
...
\begin{equation}
    g = (R, \vec{a})= (\id, \vec{a})(R,0) \qquad \vec{x}'= (R, \vec{a})\vec{x}= R\vec{x}+ \vec{a}
\end{equation}
definita in modo che prima avvenga la rotazione e successivamente la traslazione.\\
Le rototraslazioni formano ovviamente un gruppo, la cui identità: \(id = (\id,0)\), l'inversa è \((R,\vec{a})^{-1}= (R^{-1}, -R^{-1}\vec{a})\) e la composizione vale:
\[
...
\[
    (R, \vec{a})(R^{-1}, R^{-1}\vec{a}) = (\id, - RR^{-1}\vec{a}+\vec{a})= (\id,0)
\]
...
Dato che \(\mathbb{P}^2 = \id\) segue che \(\mathbb{P}= \mathbb{P}^{-1}= \mathbb{P}^\dagger\) cioé è sia unitario che autoaggiunto.\\
